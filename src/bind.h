...
struct REF_Type
{
	...
	virtual const REF_Type *flattened_type() const { return this; }
...
template <>
struct REF_TypeGetter<void>
{
	static const REF_Type *get() { return &g_void_Type; }
};
...
struct REF_TypeGetter<void *>
{
	static const REF_Type *get() { return &g_void_Ptr_Type; }
};
...
#define REF_NUMERIC_TYPE(T)                                                                                                             \
	...
	template <>                                                                                                                         \
	struct REF_TypeGetter<T>                                                                                                            \
	{                                                                                                                                   \
		static const REF_Type *get() { return &g_##T##_Type; }                                                                          \
	}
...
#define REF_PTR_TYPE(T)                                                                                                     \
	...
	struct REF_TypeGetter<T *>                                                                                              \
	{                                                                                                                       \
		static const REF_Type *get() { return &g_##T##_Ptr_Type; }                                                          \
	}
...
#define REF_PTR_PTR_TYPE(T)                                                                                                   \
	...
	struct REF_TypeGetter<T **>                                                                                               \
	{                                                                                                                         \
		static const REF_Type *get() { return &g_##T##_Ptr_Ptr_Type; }                                                        \
	}
...
template <>
struct REF_TypeGetter<bool>
{
	static const REF_Type *get() { return &g_bool_Type; }
};
REF_PTR_TYPE(bool);
...
struct char_ptr_Type : public REF_Type
{
	...
	{
		String s = from_type->to_string(from);
		*(char **)to = s.steal();
	}
...
template <>
struct REF_TypeGetter<char *>
{
	static const REF_Type *get() { return &g_char_ptr_Type; }
};
template <>
struct REF_TypeGetter<const char *>
{
	static const REF_Type *get() { return &g_char_ptr_Type; }
};
...
template <>
struct REF_TypeGetter<String>
{
	static const REF_Type *get() { return &g_String_Type; }
};
...
struct REF_Struct : public REF_Type, REF_List<REF_Struct>
{
	...
	virtual void cast(void *to, void *from, const REF_Type *from_type) const override
	{
		assert(from_type == this);
		CF_MEMCPY(to, from, size());
	}
...
struct REF_Function : public REF_List<REF_Function>
{
	...
	REF_Function(const char *name, T fn, std::initializer_list<REF_ArrayParameter> arrays)
		: m_name(name), m_sig(fn, arrays), m_fn((void (*)())fn), m_fn_wrapper(REF_ApplyWrapper<T>)
	{
	}
...
#define REF_FLAT_FLOATS(T)                                                                       \
	...
	{                                                                                            \
		...
		virtual void cast(void *to, void *from, const REF_Type *from_type) const override        \
		{                                                                                        \
			assert(from_type == REF_GetType<T>());                                               \
			*(T *)to = *(T *)from;                                                               \
		}                                                                                        \
		virtual void cleanup(void *v) const override {}                                          \
		virtual bool is_pointer() const override { return false; }                               \
		...
		{                                                                                        \
			for (int i = 0; i < N; ++i)                                                          \
				lua_pushnumber(L, ((float *)v)[i]);                                              \
		}                                                                                        \
		virtual void lua_get(lua_State *L, int index, void *v) const override                    \
		{                                                                                        \
			for (int i = 0; i < N; ++i)                                                          \
				((float *)v)[i] = (float)lua_tonumber(L, index + i);                             \
		}                                                                                        \
...
#define REF_FLAT_INTS(T)                                                                       \
	...
	{                                                                                          \
		...
		{                                                                                      \
			for (int i = 0; i < N; ++i)                                                        \
				lua_pushinteger(L, ((int *)v)[i]);                                             \
		}                                                                                      \
		virtual void lua_get(lua_State *L, int index, void *v) const override                  \
		{                                                                                      \
			for (int i = 0; i < N; ++i)                                                        \
				((int *)v)[i] = (int)lua_tointeger(L, index + i);                              \
		}                                                                                      \
		virtual int flattened_count() const override { return N; }                             \
